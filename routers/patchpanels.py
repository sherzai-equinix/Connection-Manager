...
def create_patchpanel(
    ...
    # Strip any existing PP:xxxx: or PP. prefix to avoid duplication like PP:0102:PP:0102:1406994
    if pp_number:
        pp_number = re.sub(r'^(?:PP[:.][^:]*:)*(?:PP[:.])\s*', '', pp_number, flags=re.IGNORECASE).strip()
