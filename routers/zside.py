...
def _auto_activate_cassettes(ports: list[dict], db: Session, patchpanel_id: int) -> list[dict]:
    """If any port in a cassette is occupied (via cross_connects), auto-free
    all 'unavailable' sibling ports in that cassette."""
    import re as _re

    # Find ports occupied by active cross-connects
    occ_rows = db.execute(text("""
        SELECT customer_port_label AS port_label
        FROM cross_connects
        WHERE customer_patchpanel_id = :ppid
          AND COALESCE(status, '') <> 'deinstalled'
          AND customer_port_label IS NOT NULL
    """), {"ppid": patchpanel_id}).mappings().all()
    occ_labels = {str(r["port_label"]).strip().lower() for r in occ_rows if r["port_label"]}

    # Also check connected_to field on the ports themselves
    for p in ports:
        lbl = str(p.get("port_label") or "").strip().lower()
        if p.get("connected_to") or lbl in occ_labels:
            occ_labels.add(lbl)

    # Identify cassettes with at least one occupied port
    occupied_cassettes: set[str] = set()
    for lbl in occ_labels:
        m = _re.match(r'^(\d+[a-d])\d+$', lbl)
        if m:
            occupied_cassettes.add(m.group(1).upper())

    if not occupied_cassettes:
        return ports

    # Auto-free unavailable sibling ports
    for p in ports:
        st = str(p.get("status") or "").lower()
        if st not in ("unavailable", ""):
            continue
        label = str(p.get("port_label") or "")
        m = _re.match(r'^(\d+[A-D])\d+$', label)
...
def get_patchpanel_ports(patchpanel_id: int, db: Session = Depends(get_db)):
    ports = crud.list_patchpanel_ports(db, patchpanel_id)
    return _auto_activate_cassettes(ports, db, patchpanel_id)
